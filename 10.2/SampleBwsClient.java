...
public class SampleBwsClient
{
	...
	private final static String CLIENT_VERSION = "10.2.0";
	...
	private static DeviceActivationType CREATE_NEW_USER_DEVICE_ACTIVATION_TYPE = null;
	...
	// Used to determine if the domain has a default device activation type.
	private static boolean DEFAULT_DEVICE_ACTIVATION_TYPE_EXISTS = false;
	...
	public static User getUser()
	{
		...
		/* 
		 * Check if the value of the variable "DISPLAY_USER_DETAIL_EMAIL" is enclosed in double-quotes.
		 * If it is, then the search will look for an email address that exactly matches this value.
		 * Otherwise, this value will be used in a substring match (or a prefix match if the value
		 * starts with a double-quote).
		 */
		if (!DISPLAY_USER_DETAIL_EMAIL.startsWith("\"") || !DISPLAY_USER_DETAIL_EMAIL.endsWith("\""))
		{
			logMessage(	"Warning: Email Address \"%s\" is not enclosed in double-quotes. A substring match will occur "
					+ "(or a prefix match if the value starts with a double-quote).", DISPLAY_USER_DETAIL_EMAIL);
		}
	...
	public static boolean createUser()
	{
		...
		if(DEFAULT_DEVICE_ACTIVATION_TYPE_EXISTS)
		{
			logMessage("Device activation type set to \"%s\"", CREATE_NEW_USER_DEVICE_ACTIVATION_TYPE.getValue());
			newUser.setActivationTypeForNewAndReactivatedDevices(CREATE_NEW_USER_DEVICE_ACTIVATION_TYPE);			
		}
	...
	public static boolean getSystemInfo()
	{
		...
		if (response.getReturnStatus().getCode().equals("SUCCESS"))
		{
			if (response.getProperties() != null && !response.getProperties().isEmpty())
			{
				...
				{
					if(property.getName().equals("Default Device Activation Type")){						
						DEFAULT_DEVICE_ACTIVATION_TYPE_EXISTS = true;
					}
	 ...
	 *  
	 * @return Returns a DeviceActivationType of type BLACKBERRY_BALANCE.
	 * 
	 ******************************************************************************************************************* 
	 */
	public static DeviceActivationType createBlackBerryBalanceDeviceActivationType(){
		...
		blackberryBalance.setBLACKBERRYBALANCE(true);
		blackberryBalance.setValue("BLACKBERRY_BALANCE");		
			...
			if (setup())
			{
				...
				{					
					if (createNewUser)
					{
						...
						{
							// Device activation type used to create a new user with the createUsers() API call.
							CREATE_NEW_USER_DEVICE_ACTIVATION_TYPE = createBlackBerryBalanceDeviceActivationType();
						}
